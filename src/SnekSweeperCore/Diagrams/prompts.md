...
- [x] `GridLogic` 状态机重构（已落地，Output+回调机制）：
  ...
  - 三个 instantiated 状态合并为 `Initializing`：`Data.PendingFirstInput` 显式存首次输入，按 `LoadLevelSource` 决定接受规则/光标锁定/StartInfo，发 `Output.RestoreGrid`（续局提前恢复）/ `Output.LayMinesAt`（首次点击布雷）→ 绑定层初始化 → 回调 `Input.InitCompleted` → `Running.OnEnter` 消费 pending
